...
class LoginService implements LoginBASE{
  ...
  Future<LoginModel> currentUser() async {
    if(_firebaseAuth.currentUser !=null){
      ...
      LoginModel? girisYapanKullanici = LoginModel(email: user!.email, id: user.uid);
      return girisYapanKullanici;
    }else{
  ...
  Future<RegisterFirestoreModel> girisYapmisKullaniciBilgileriniCek({required String email}) async {
    QuerySnapshot<Map<String,dynamic>> kullaniciSorgu = await  _firebaseFirestore.collection("kullanicilar").where("kullanici_email", isEqualTo: email).get();
